...
async function getSDKDependencies() {
    ...
    packageJsonFiles.forEach((packageJsonFile) => {
        ...
        let SDKPkg = path.basename(path.dirname(packageJsonFile)) === "sdk" ? "TeamsFx SDK" : "TeamsFx React SDK";
        ...
        for (dependency in dependencies) {
            if (dependenciesMap.has(dependency)) {
                ...
                dependenciesMap.set(dependency, [{ SDKPkg, version: dependencies[dependency] }]);
            }
        }
    });
...
async function getDotnetSDKDependencies() {
    var dependenciesMap = new Map();
    const csprojContent = fs.readFileSync(`${repoRoot}/packages/dotnet-sdk/src/TeamsFx/Microsoft.TeamsFx.csproj`, "utf8");
    ...
    for (const csDependency of csDependencies) {
        if (dependenciesMap.has(csDependency.name)) {
            ...
            dependenciesMap.set(csDependency.name, [{ SDKPkg: "TeamsFx .NET SDK", version: csDependency.version }]);
        }
    }
