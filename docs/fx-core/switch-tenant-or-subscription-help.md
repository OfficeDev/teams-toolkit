...
After switching Microsoft M365 tenant for preparing Teams app dependencies in Visual Studio, you may notice issues when local debugging with a Teams app requiring AAD resources including receiving 401 response when sending a bot command or could not authorize to get the user's profile photo in the tab like the image shown below. 
