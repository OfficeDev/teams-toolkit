...
async function getRealValue(
    ...
    if (typeof defaultValue === "string") {
        ...
        if (defstr === "$parent") {
            ...
            defstr.startsWith("$parent.") &&
            parentValue instanceof Object
        ) {
        ...
        if (remoteFuncExecutor && func && func.method) {
            const res = await remoteFuncExecutor(
                defaultValue as Func,
                inputs,
                stage,
            );
            if (res.isOk()) {
...
function isAutoSkipSelect(q: Question): boolean {
    if (q.type === NodeType.singleSelect) {
        ...
        if (
            select.skipSingleOption &&
            select.option instanceof Array &&
            options.length === 1
        ) {
            return true;
        }
...
const questionVisitor: QuestionVistor = async function (
    ...
    if (type === NodeType.func) {
        ...
        if (question.default) {
            defaultValue = await getRealValue(
                parentValue,
                question.default,
                inputs,
                stage,
        ...
        if (
            ...
            const inputQuestion:
                | TextInputQuestion
                | NumberInputQuestion = question as
                | TextInputQuestion
                | NumberInputQuestion;
            const validationFunc = inputQuestion.validation
                ? getValidationFunction(
                      inputQuestion.validation,
                      inputs,
                      stage,
                      remoteFuncExecutor,
                  )
            ...
            const selectQuestion:
                | SingleSelectQuestion
                | MultiSelectQuestion = question as
                | SingleSelectQuestion
                | MultiSelectQuestion;
            let option: Option = [];
            ...
            if (
                type === NodeType.singleSelect &&
                (skipSingleOption === undefined || skipSingleOption === true) &&
                option.length === 1
            ) {
                ...
                if (selectQuestion.returnObject) {
                    ...
                    return {
                        type: InputResultType.pass,
                        result: optionIsString
                            ? option[0]
                            : (option[0] as OptionItem).id,
                    };
                }
            }
            return await ui.showQuickPick({
                ...
                onDidChangeSelection:
                    type === NodeType.multiSelect
                        ? (selectQuestion as MultiSelectQuestion)
                              .onDidChangeSelection
                        : undefined,
            });
...
export async function traverse(
    ...
    while (stack.length > 0) {
        ...
        if (curr.data.type !== NodeType.group) {
            ...
            const parentValue =
                parent && parent.data.type !== NodeType.group
                    ? parent.data.value
                    : undefined;
