...
export interface ResourcePlugin {
   ...
   *
   * @returns {Json} part of {@link EnvProfile}
   */
  provisionResource?: (
    ctx: Context,
   ...
   * @param {Readonly<ProvisionInputs>} inputs - inputs injected by Toolkit runtime and solution.
   * @param {EnvConfig} envConfig - model for config.${env}.json, in which, user can customize some inputs for provision
   * @param {EnvProfile} envProfile - the profile (persist by core as `profile.${env}.json`) containing provision outputs
   * @param {TokenProvider} tokenProvider - Tokens for Azure and AppStudio
   *
  ...
  package?: (
    ...
    inputs: Inputs, 
    envConfig: EnvConfig,
    envProfile: EnvProfile) => Promise<Result<Void, FxError>>;
   ...
   * @param {Context} ctx - plugin's runtime context shared by all lifecycles.
   * @param {Inputs} inputs - system inputs.
   * @param {LocalSettings} localSettings - local debug settings generated by {@link provisionLocalResource}
   * @param {TokenProvider} tokenProvider - Tokens for Azure and AppStudio
   *
   * @returns the output values of local debug settings. Toolkit will persist them
   *          and pass them to {@link configureLocalResource}. The output will be persisted but not in the same file as provision's output.
   */
   ...
   *
   * @param {Context} ctx - plugin's runtime context shared by all lifecycles.
   * @param {LocalSettings} localSettings - local debug settings generated by {@link scaffoldSourceCode}
   * @param {TokenProvider} tokenProvider - Tokens for Azure and AppStudio
   *
   * @returns the output values of local debug settings, which will be reused in next local debug task
   */
