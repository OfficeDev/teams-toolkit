  ...
  describe("traverse()", () => {
    ...
    it("success: auto skip single option select", async () => {
      ...
      for (let i = 1; i <= num; ++i) {
        ...
        if(i % 2 === 0)
          question.staticOptions = [`mocked value of ${name}`]; 
        else{
    ...
    it("success: flat sequence with back operation", async () => {
      ...
      const inputs = createInputs();
      sandbox.stub(mockUI, "selectOption").callsFake(async (config: SingleSelectConfig):Promise<Result<SingleSelectResult,FxError>> => {
        actualSequence.push(config.name);
    ...
    it("success: SingleSelectQuestion, MultiSelectQuestion", async () => {
      ...
      sandbox.stub(mockUI, "selectOptions").callsFake(async (config: MultiSelectConfig):Promise<Result<MultiSelectResult,FxError>> => {
        actualSequence.push(config.name);
      ...
      assert.deepEqual((inputs["3"] as StaticOptions)[0],{id:`mocked value of 3`, label:`mocked value of 3`});
      assert.deepEqual((inputs["4"] as StaticOptions)[0],{id:`mocked value of 4`, label:`mocked value of 4`});
      assert.sameOrderedMembers(expectedSequence,actualSequence);
    });
