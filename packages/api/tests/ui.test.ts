...
import { GroupOfTasks, InputTextConfig, InputTextResult, MultiSelectConfig, MultiSelectResult, RunnableTask, SelectFileConfig, SelectFileResult, SelectFilesConfig, SelectFilesResult, SelectFolderConfig, SelectFolderResult, SingleSelectConfig, SingleSelectResult, TaskConfig, UserInteraction } from "../src/qm/ui";
...
describe("UserInteraction(UI) - GroupOfTasks", () => {
  it("case 1: seq=true, fastFail=true, sub-task return error", async () => {
    ...
    const res = await MockUI.runWithProgress(group, {showProgress:showProgress, cancellable:cancellable});
  ...
  it("case 4: seq=true, fastFail=false, sub-task throw error", async () => {
    ...
    const group = new GroupOfTasks<undefined>([task1,task2,task3], {sequential:sequential, fastFail:fastFail});
    const res = await MockUI.runWithProgress(group, {showProgress:showProgress, cancellable:cancellable});
    assert.isTrue(res.isOk() && Array.isArray(res.value) && res.value[1].isErr() && res.value[1].error === UserCancelError);
  });
