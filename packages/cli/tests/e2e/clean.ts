...
export async function cleanUpAad(contains: string, hours?: number, retryTimes = 2): Promise<void> {
...
export async function cleanUpResourceGroup(
  ...
  console.log(`[Successfully] clean up ${promises.length} Azure resource groups.`);
}
...
(async () => {
  ...
  const promise4 = cleanUpResourceGroup("teamsfxt");
