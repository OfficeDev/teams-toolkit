...
import CICD from "../../../src/cmds/cicd";
...
describe("CICD Command Tests", function () {
  ...
  it("Builder Check", () => {
    ...
    expect(registeredCommands).deep.equals(["cicd"]);
  });
  ...
  it("CICD Command Running Check", async () => {
    ...
    const cmd = new CICD();
    await cmd.handler({});
    expect(telemetryEvents).deep.equals([TelemetryEvent.CICDStart, TelemetryEvent.CICD]);
  });
});
