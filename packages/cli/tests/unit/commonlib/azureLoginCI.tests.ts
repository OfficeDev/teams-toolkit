...
describe("Azure Service Principal login Tests", function () {
  ...
  before(async () => {
    ...
    sandbox
      .stub(msRestNodeAuth, "loginWithServicePrincipalCertificate")
      .callsFake(async (clientId: string, certificateStringOrFilePath: string, domain: string) => {
        return new msRestNodeAuth.ApplicationTokenCertificateCredentials(
          "clientId",
          "tenantId",
          "secret",
          "thumbprint"
