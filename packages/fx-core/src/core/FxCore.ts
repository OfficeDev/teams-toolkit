...
import { CoreHookContext, VersionCheckRes } from "./types";
...
export class FxCore implements v3.ICore {
  ...
  async projectVersionCheck(inputs: Inputs): Promise<Result<VersionCheckRes, FxError>> {
    ...
    const versionInfo =
      (await tryGetVersionInfoV2(projectPath)) ||
      (await tryGetVersionInfoV3(projectPath)) ||
      (await tryGetVersionInfoV3Abandoned(projectPath));
    if (versionInfo) {
