...
export class Executor {
    ...
    static async localDebug(
        ...
        return ctx.selectedSolution!.localDebug(
            tools.contextTosolutionContext(ctx, answers),
        );
    }
            ...
            for (const [k, v] of ctx.globalSolutions) {
                if (v.getQuestions) {
                    ...
                    if (solutionNode.data)
                        select_solution.addChild(solutionNode);
                }
            }
    ...
    static async getQuestionsForUserTask(
        ...
        if ("" !== namespace && array.length > 0) {
            ...
            if (solution && solution.getQuestionsForUserTask) {
                const solutioContext = tools.contextTosolutionContext(ctx);
                return await solution.getQuestionsForUserTask(
                    func,
                    solutioContext,
                );
            }
        ...
        return err(
            returnUserError(
                new Error(
                    `getQuestionsForUserTaskRouteFailed:${JSON.stringify(
                        func,
                    )}`,
                ),
                error.CoreSource,
    ...
    static async executeUserTask(
        ...
        if ("" !== namespace && array.length > 0) {
            ...
            if (solution && solution.executeUserTask) {
                const solutioContext = tools.contextTosolutionContext(
                    ctx,
                    answer,
                );
    ...
    static async callFunc(
        ...
        if (!namespace || "" === namespace || array.length === 0) {
            if (func.method === "validateFolder") {
                ...
                return await this.validateFolder(
                    ctx,
                    func.params as string,
                    answer,
                );
            }
        } else {
    ...
    static async create(
        ...
        const result = await targetCtx.selectedSolution!.create(
            tools.contextTosolutionContext(targetCtx, answers),
        );
        if (result.isErr()) {
            ctx.logProvider?.info(
                `[Core] create - call solution.create() failed!`,
            );
            return result;
        }
        ctx.logProvider?.info(
            `[Core] create - call solution.create() success!`,
        );
        ...
        ctx.logProvider?.info(
            `[Core] create - create basic folder with configs`,
        );
    ...
    static async scaffold(
        ...
        return ctx.selectedSolution!.scaffold(
            tools.contextTosolutionContext(ctx, answers),
        );
    }
    ...
    static async update(
        ...
        return ctx.selectedSolution!.update(
            tools.contextTosolutionContext(ctx, answers),
        );
    }
    ...
    static async provision(
        ...
        return ctx.selectedSolution!.provision(
            tools.contextTosolutionContext(ctx, answers),
        );
    }
    ...
    static async deploy(
        ...
        return ctx.selectedSolution!.deploy(
            tools.contextTosolutionContext(ctx, answers),
        );
    }
