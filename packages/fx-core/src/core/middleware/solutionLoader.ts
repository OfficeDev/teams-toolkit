...
import { getAllSolutionPlugins, getAllSolutionPluginsV2, getSolutionPluginByName, getSolutionPluginV2ByName } from "../SolutionPluginContainer";

export function SolutionLoaderMW(): Middleware {
  return async (ctx: CoreHookContext, next: NextFunction) => {
    if (ctx.projectSettings) {
      if (isV2()) {
        const solution = getSolutionPluginV2ByName(
