...
import { err, FxError, ok, Result, returnSystemError, VsCode } from "@microsoft/teamsfx-api";
...
export class VscodeManager implements VsCode {
  ...
  public async addSettings(settings: any): Promise<Result<null, FxError>> {
    if (!this.workspace) {
      return err(
        returnSystemError(
          new Error(`Failed to add settings due to empty workspace directory`),
          "Core",
          "name"
        )
      );
  ...
  private async appendDevSettings(
    devFile: string,
    field: string,
    settings: any
