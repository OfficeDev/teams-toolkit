...
export class ApimManager {
    ...
    public async scaffold(app: Readonly<TeamsAppManifest>, projectRootPath: string) {
    ...
    public async provision(apimConfig: IApimPluginConfig, solutionConfig: ISolutionConfig, appName: string) {
        ...
        const apimServiceName =
            apimConfig.serviceName ?? NameSanitizer.sanitizeApimName(appName, solutionConfig.resourceNameSuffix);
        ...
        const productId =
            apimConfig.productId ?? NameSanitizer.sanitizeProductId(appName, solutionConfig.resourceNameSuffix);
    ...
    public async postProvision(
        ...
        const clientId = AssertConfigNotEmpty(TeamsToolkitComponent.ApimPlugin, ApimPluginConfigKeys.apimClientAADClientId, apimConfig.apimClientAADClientId);
        const clientSecret =  AssertConfigNotEmpty(TeamsToolkitComponent.ApimPlugin, ApimPluginConfigKeys.apimClientAADClientSecret, apimConfig.apimClientAADClientSecret);

        const oAuthServerId =
            apimConfig.oAuthServerId ?? NameSanitizer.sanitizeOAuthServerId(appName, solutionConfig.resourceNameSuffix);
    ...
    public async deploy(
        ...
        const apiDocumentPath = AssertConfigNotEmpty(TeamsToolkitComponent.ApimPlugin, ApimPluginConfigKeys.apiDocumentPath, apimConfig.apiDocumentPath);
        ...
        let openApiDocument = await this.openApiProcessor.loadOpenApiDocument(apiDocumentPath, projectRootPath);
        ...
        await this.apimService.createVersionSet(
            resourceGroupName,
            apimServiceName,
            versionSetId,
