...
export class ApimService {
    ...
    public async createService(resourceGroupName: string, serviceName: string, location: string): Promise<void> {
        ...
        const fn = () =>
            this.apimClient.apiManagementService.createOrUpdate(resourceGroupName, serviceName, apimService);
    ...
    public async listService(): Promise<Array<IApimServiceResource>> {
        ...
        let result = serviceList.map((response) => this.convertApimServiceResource(response));
    ...
    public async listApi(
        ...
        const resourceId = versionSetId
            ...
            : undefined;
        const fn = () =>
            this.apimClient.api.listByService(resourceGroupName, serviceName, { expandApiVersionSet: true });
        ...
        let result = apiList.filter((x) => !!resourceId && x.apiVersionSet?.id === resourceId);
    ...
    public async createVersionSet(
        ...
        let originVersionSet = await this.getVersionSet(resourceGroupName, serviceName, versionSetId);
        if (originVersionSet) {
            return;
        }
        ...
        const fn = () =>
            this.apimClient.apiVersionSet.createOrUpdate(resourceGroupName, serviceName, versionSetId, newVersionSet);
    ...
    if ('code' in error) {
        if (error.code === 'ValidationError') {
            if (error.body.details instanceof Array) {
                for (let detailError of error.body.details) {
