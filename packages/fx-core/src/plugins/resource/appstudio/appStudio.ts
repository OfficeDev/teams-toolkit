...
export namespace AppStudioClient {
      ...
      if (response && response.data) {
        ...
        if (app && app.teamsAppId && app.teamsAppId === teamsAppId) {
          ...
          throw AppStudioResultFactory.SystemError(
            ...
            AppStudioError.TeamsAppUpdateIDNotMatchError.message(
              teamsAppId,
              app.teamsAppId
            )
          );
  ...
  export async function publishTeamsApp(
    ...
    try {
      ...
      const appCatalogAppId = await getAppByTeamsAppId(
        teamsAppId,
        appStudioToken
      );
      ...
      if (appCatalogAppId) {
        ...
        response = await requester.post(
          `/api/publishing/${teamsAppId}/appdefinitions`,
          file,
  ...
  async function getAppByTeamsAppId(
    teamsAppId: string,
    appStudioToken: string
