...
export class FunctionDeploy {
  ...
  public static async hasUpdatedContent(
    ...
    try {
      ...
      const gitIgnore = await this.prepareIgnore(
        componentPath,
        FunctionPluginPathInfo.gitIgnoreFileName
      );
  ...
  public static async deployFunction(
    ...
    const publishRelativePath: string =
      LanguageStrategyFactory.getStrategy(language).deployFolderRelativePath;
    const publishAbsolutePath: string = path.join(componentPath, publishRelativePath);

    const zip: AdmZip = await runWithErrorCatchAndThrow(
      new ZipError(),
      async () =>
        await step(
          StepGroup.DeployStepGroup,
          DeploySteps.generateZip,
          async () => await this.generateFunctionZip(publishAbsolutePath)
        )
    );

    const publishCred: WebAppsListPublishingCredentialsResponse = await runWithErrorCatchAndThrow(
    ...
    await runWithErrorCatchAndThrow(
      new FunctionAppOpError("restart"),
      async () =>
        await step(
          StepGroup.DeployStepGroup,
          DeploySteps.restart,
  ...
  private static async saveDeploymentInfo(
    ...
    const deploymentZipCache = path.join(
      deploymentInfoDir,
      FunctionPluginPathInfo.funcDeploymentZipCacheFileName
    );
  ...
  private static async loadLastDeploymentZipCache(
    componentPath: string
  ): Promise<AdmZip | undefined> {
    const deploymentInfoDir = path.join(
      componentPath,
      FunctionPluginPathInfo.funcDeploymentFolderName
    );
