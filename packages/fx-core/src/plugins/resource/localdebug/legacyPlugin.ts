...
export class legacyLocalDebugPlugin {
  ...
  public static async postLocalDebug(ctx: PluginContext): Promise<Result<any, FxError>> {
    ...
    if (ctx.answers?.platform === Platform.VSCode || ctx.answers?.platform === Platform.CLI) {
      ...
      const aadConfigs = ctx.envInfo.state.get(AadPlugin.Name);
      const runtimeConnectorConfigs = ctx.envInfo.state.get(RuntimeConnectorPlugin.Name);
      ...
      if (includeBot) {
        // bot local env
        const botConfigs = ctx.envInfo.state.get(BotPlugin.Name);
        if (isMigrateFromV1) {
