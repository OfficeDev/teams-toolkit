...
export async function deployArmTemplates(ctx: SolutionContext): Promise<Result<void, FxError>> {
  ...
  try {
    ...
    if (result.isOk()) {
      ...
      // The innerError will assigned a string value to describe the deploymentError and be send as a telemetry property.
      // Otherwise, the innerError is Error type generated by default, we will not use it here.
