...
export class TeamsAppSolution implements Solution {
    ...
    private async canPublish(ctx: SolutionContext): Promise<Result<TeamsAppManifest | undefined, FxError>> {
        const solutionConfig = ctx.config;
        ...
        const isProvisionSucceeded = this.checkWetherProvisionSucceeded(solutionConfig);
        ...
        let manifestString: string | undefined = undefined;
            if (this.spfxSelected(ctx)) {
                ...
                manifestString = solutionConfig.get(GLOBAL_CONFIG)?.getString(PROVISION_MANIFEST);
            }
            if (!manifestString) {
                return err(
                    returnSystemError(
    ...
    async publish(ctx: SolutionContext): Promise<Result<any, FxError>> {
        ...
        const maybeManifest = await this.canPublish(ctx);
