...
export class TeamsAppSolution implements Solution {
    ...
    private canPublish(ctx: SolutionContext): Result<TeamsAppManifest | undefined, FxError> {
        ...
        return this.checkWhetherSolutionIsIdle().andThen((_) => {
            ...
            if (this.spfxSelected(ctx)) {
                manifestString = fs.readFileSync(`${ctx.root}/.${ConfigFolderName}/${REMOTE_MANIFEST}`).toString();
            } else {
    ...
    async publish(ctx: SolutionContext): Promise<Result<any, FxError>> {
        ...
        const maybeManifest = this.canPublish(ctx);
