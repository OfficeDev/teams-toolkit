...
export class TeamsAppSolution implements Solution {
  ...
  private getPluginAndContextArray(
    ...
    return selectedPlugins.map((plugin) => [
      plugin,
      getPluginContext(ctx, plugin.name, manifest ?? this.manifest),
    ]);
  }
  ...
  async create(ctx: SolutionContext): Promise<Result<any, FxError>> {
    ...
    const defaultIconPath = path.join(
      __dirname,
      "../../../../templates/plugins/solution/defaultIcon.png"
    );
    await fs.copy(
      defaultIconPath,
      `${ctx.root}/.${ConfigFolderName}/color.png`
    );
    await fs.copy(
      defaultIconPath,
      `${ctx.root}/.${ConfigFolderName}/outline.png`
    );
    if (!this.spfxSelected(ctx.config)) {
      ...
      await fs.writeFile(
        `${ctx.root}/.${ConfigFolderName}/manifest.remote.json`,
        JSON.stringify(this.manifest, null, 4)
      );
      await fs.writeJSON(
        `${ctx.root}/permissions.json`,
        DEFAULT_PERMISSION_REQUEST,
        { spaces: 4 }
      );
      return this.updatePermissionRequest(ctx.root, ctx.config);
    } else {
      this.manifest = await ((this
        .spfxPlugin as unknown) as SpfxPlugin).getManifest();
      await fs.writeFile(
        `${ctx.root}/.${ConfigFolderName}/manifest.remote.json`,
        JSON.stringify(this.manifest, null, 4)
      );
  ...
  private async reloadManifest(
    ctx: SolutionContext
  ): Promise<Result<any, FxError>> {
    // read manifest
    if (!this.spfxSelected(ctx.config)) {
      try {
        this.manifest = await fs.readJson(
          `${ctx.root}/.${ConfigFolderName}/manifest.remote.json`
        );
        if (!this.manifest) {
          return err(
  ...
  reloadPlugins(solutionConfig: SolutionConfig, answer: ConfigMap): void {
    ...
    if (
      cap?.includes(BotOptionItem.label) ||
      cap?.includes(MessageExtensionItem.label)
    ) {
  ...
  async update(ctx: SolutionContext): Promise<Result<any, FxError>> {
    const selectedPlugins = ctx.config
      .get(GLOBAL_CONFIG)
      ?.getStringArray(SELECTED_PLUGINS);
    ...
    const alreadyHaveFunction = selectedPlugins?.includes(
      this.functionPlugin.name
    );
    ...
    const oldResources = ctx.answers?.get(
      AzureSolutionQuestionNames.AzureResources
    ) as string[];
    const addResources = ctx.answers?.get(
  ...
  async scaffold(ctx: SolutionContext): Promise<Result<any, FxError>> {
    ...
    const preScaffoldWithCtx: LifecyclesWithContext[] = pluginsWithCtx.map(
      ([plugin, context]) => {
        return [plugin?.preScaffold?.bind(plugin), context, plugin.name];
      }
    );
    const scaffoldWithCtx: LifecyclesWithContext[] = pluginsWithCtx.map(
      ([plugin, context]) => {
        return [plugin?.scaffold?.bind(plugin), context, plugin.name];
      }
    );
    const postScaffoldWithCtx: LifecyclesWithContext[] = pluginsWithCtx.map(
      ([plugin, context]) => {
  ...
  private async updatePermissionRequest(
    ...
    config
      .get(GLOBAL_CONFIG)
      ?.set(PERMISSION_REQUEST, JSON.stringify(permissionRequest));
    return ok(config);
  }
  ...
  private async createAndConfigTeamsManifest(
    ...
    if (
      selectedPlugins.some((plugin) => plugin.name === this.fehostPlugin.name)
    ) {
      endpoint = ctx.config
        .get(this.fehostPlugin.name)
        ?.getString(FRONTEND_ENDPOINT);
      domain = ctx.config
        .get(this.fehostPlugin.name)
        ?.getString(FRONTEND_DOMAIN);
    ...
    const clientId = ctx.config
      .get(this.aadPlugin.name)
      ?.getString(AAD_REMOTE_CLIENT_ID);
    ...
    if (selectedPlugins.some((plugin) => plugin.name === this.botPlugin.name)) {
      const capabilities = ctx.answers?.getStringArray(
      ...
      if (!hasBot && !hasMsgExt) {
        return err(
          returnSystemError(
            new Error(
              "One of bot and Message Extension is expected to be selected"
            ),
            "Solution",
      ...
      if (hasMsgExt) {
        const composeExtensions = ctx.config
          .get(this.botPlugin.name)
          ?.getString(COMPOSE_EXTENSIONS);
        if (!composeExtensions) {
          return err(
            returnSystemError(
              new Error(
                `key "${COMPOSE_EXTENSIONS}" not found in bot plugin's conifg`
              ),
              "Solution",
              SolutionError.BotInternalError
            )
    ...
    const webApplicationInfoResource: string | undefined = ctx.config
      .get(this.aadPlugin.name)
      ?.getString(WEB_APPLICATION_INFO_SOURCE);
    if (webApplicationInfoResource) {
      ctx.logProvider?.debug(
        `Succeed to get webApplicationInfoResource: ${webApplicationInfoResource}`
      );
    } else {
      ctx.logProvider?.debug(
        `Failed to get webApplicationInfoResource from aad by key ${WEB_APPLICATION_INFO_SOURCE}.`
      );
      return err(
        returnSystemError(
    ...
    const teamsAppId = ctx.config
      .get(GLOBAL_CONFIG)
      ?.getString(REMOTE_TEAMS_APP_ID);
    if (!teamsAppId) {
      ...
      ctx.config
        .get(GLOBAL_CONFIG)
        ?.set(PROVISION_MANIFEST, JSON.stringify(updatedManifest));
      ...
      ctx.logProvider?.info(
        `Teams app updated ${JSON.stringify(updatedManifest)}`
      );
      return ok(appDefinition);
    }
  }
  ...
  private checkWetherProvisionSucceeded(
    ...
    return !!solutionConfig
      .get(GLOBAL_CONFIG)
      ?.getBoolean(SOLUTION_PROVISION_SUCCEEDED);
  }
  ...
  async provision(ctx: SolutionContext): Promise<Result<any, FxError>> {
    if (this.spfxSelected(ctx.config)) {
      return err(
        returnUserError(
          new Error(
            "SPFx project has no provision task, you can directly deploy it."
          ),
          "Solution",
  ...
  async doProvision(ctx: SolutionContext): Promise<Result<any, FxError>> {
    ...
    const loadManifestResult = await this.reloadManifestAndCheckRequiredFields(
      ctx
    );
    if (loadManifestResult.isErr()) {
      return loadManifestResult;
    ...
    const preProvisionWithCtx: LifecyclesWithContext[] = pluginsWithCtx.map(
      ([plugin, context]) => {
        return [plugin?.preProvision?.bind(plugin), context, plugin.name];
      }
    );
    const provisionWithCtx: LifecyclesWithContext[] = pluginsWithCtx.map(
      ([plugin, context]) => {
    ...
    return executeLifecycles(
      ...
      async () => {
        ...
        if (
          selectedPlugins.some((plugin) => plugin.name === this.aadPlugin.name)
        ) {
  ...
  private canPublish(
    solutionConfig: SolutionConfig,
    platform?: Platform
  ): Result<TeamsAppManifest | undefined, FxError> {
    ...
    return this.checkWhetherSolutionIsIdle()
      .andThen((_) => {
        return this.checkWetherProvisionSucceeded(solutionConfig)
          ? ok(Void)
          : err(
      ...
      .andThen((_) => {
        const manifestString = solutionConfig
          .get(GLOBAL_CONFIG)
          ?.getString(PROVISION_MANIFEST);
        if (!manifestString) {
          return err(
            returnSystemError(
  ...
  private async doDeploy(ctx: SolutionContext): Promise<Result<any, FxError>> {
    ...
    ctx.logProvider?.info(
      `[Solution] Selected plugins to deploy:${JSON.stringify(
        pluginsToDeploy.map((p) => p.name)
      )}`
    );
    const pluginsWithCtx: PluginsWithContext[] = this.getPluginAndContextArray(
      ctx,
    ...
    const preDeployWithCtx: LifecyclesWithContext[] = pluginsWithCtx.map(
      ([plugin, context]) => {
        return [plugin?.preDeploy?.bind(plugin), context, plugin.name];
      }
    );
    const deployWithCtx: LifecyclesWithContext[] = pluginsWithCtx.map(
      ([plugin, context]) => {
        return [plugin?.deploy?.bind(plugin), context, plugin.name];
      }
    );
    const postDeployWithCtx: LifecyclesWithContext[] = pluginsWithCtx.map(
      ([plugin, context]) => {
  ...
  async publish(ctx: SolutionContext): Promise<Result<any, FxError>> {
    ...
    try {
      ...
      const publishWithCtx: LifecyclesWithContext[] = pluginsWithCtx.map(
        ([plugin, context]) => {
    ...
    if (stage === Stage.create) {
      ...
      if (featureFlag && this.botPlugin.getQuestions) {
        ...
        if (res.value) {
          ...
          botGroup.condition = {
            containsAny: [BotOptionItem.label, MessageExtensionItem.label],
          };
      ...
      if (
        capabilities &&
        capabilities?.includes(TabOptionItem.label) &&
        htype === HostTypeOptionAzure.label
      ) {
        const selectedPlugins = ctx.config
          .get(GLOBAL_CONFIG)
          ?.getStringArray(SELECTED_PLUGINS);
  ...
  private async createAndUpdateApp(
    ...
    await logProvider?.debug(
      `${type} appDefinition: ${JSON.stringify(appDefinition)}`
    );
    if (appStudioToken === undefined || appStudioToken.length === 0) {
      return err(
        returnSystemError(
    ...
    const teamsAppId = await AppStudio.createApp(
      appDefinition,
      appStudioToken,
      logProvider
    );
  ...
  async localDebug(ctx: SolutionContext): Promise<Result<any, FxError>> {
    ...
    const localDebugWithCtx: LifecyclesWithContext[] = pluginsWithCtx.map(
      ([plugin, context]) => {
    ...
    const manifestTpl = (
      await fs.readFile(`${ctx.root}/.${ConfigFolderName}/manifest.remote.json`)
    ).toString();
    ...
    const localTeamsAppID = ctx.config
      .get(GLOBAL_CONFIG)
      ?.getString(LOCAL_DEBUG_TEAMS_APP_ID);
    ...
    if (localTeamsAppID) {
      ...
      ctx.config
        .get(GLOBAL_CONFIG)
        ?.set(LOCAL_DEBUG_TEAMS_APP_ID, maybeTeamsAppId.value);
    }
  ...
  private loadTeamsAppTenantId(
    ...
    return this.parseTeamsAppTenantId(appStudioToken).andThen(
      (teamsAppTenantId) => {
  ...
  private getLocalDebugConfig(
    ...
    SystemError
  > {
    const localTabEndpoint = config
      .get(this.localDebugPlugin.name)
      ?.getString(LOCAL_DEBUG_TAB_ENDPOINT);
    const localTabDomain = config
      .get(this.localDebugPlugin.name)
      ?.getString(LOCAL_DEBUG_TAB_DOMAIN);
    const localAADId = config
      .get(this.aadPlugin.name)
      ?.getString(LOCAL_DEBUG_AAD_ID);
    const localBotDomain = config
      .get(this.localDebugPlugin.name)
      ?.getString(LOCAL_DEBUG_BOT_DOMAIN);
    const bots = config.get(this.botPlugin.name)?.getString(BOTS);
    const composeExtensions = config
      .get(this.botPlugin.name)
      ?.getString(COMPOSE_EXTENSIONS);
    ...
    const webApplicationInfoResource = config
      .get(this.aadPlugin.name)
      ?.getString(LOCAL_WEB_APPLICATION_INFO_SOURCE);
    if (!webApplicationInfoResource) {
      return err(
        returnSystemError(
          new Error("Failed to get webApplicationInfoResource"),
    ...
    if (!localTabEndpoint && !bots && !composeExtensions) {
      return err(
        returnSystemError(
          new Error(
            `${LOCAL_DEBUG_TAB_ENDPOINT}, ${BOTS}, ${COMPOSE_EXTENSIONS} are all missing`
          ),
          "Solution",
          SolutionError.GetLocalDebugConfigError
        )
    ...
    if (
      (localTabEndpoint && !localTabDomain) ||
      (!localTabEndpoint && localTabDomain)
    ) {
      return err(
        returnSystemError(
          new Error(
            `Invalid config for tab: ${LOCAL_DEBUG_TAB_ENDPOINT}=${localTabEndpoint} ${LOCAL_DEBUG_TAB_DOMAIN}=${localTabDomain}`
          ),
          "Solution",
          SolutionError.GetLocalDebugConfigError
        )
    ...
    if (array.length === 2) {
      ...
      if (plugin && plugin.callFunc) {
        ...
        if (func.method === "aadUpdatePermission") {
          const result = await this.updatePermissionRequest(
            ctx.root,
            ctx.config
          );
          if (result.isErr()) {
        ...
        if (!ctx.config.get(GLOBAL_CONFIG)?.getString("subscriptionId")) {
          ...
          if (azureToken === undefined) {
            return err(
              returnUserError(
                new Error(
                  "Please login to azure using Azure Account Extension"
                ),
                "Solution",
                SolutionError.NotLoginToAzure
              )
          ...
          const result = await askSubscription(
            ctx.config,
            azureToken,
            ctx.dialog
          );
  ...
  async executeUserTask(
    ...
    if (namespace.includes("solution") && method === "registerTeamsAppAndAad") {
      const maybeParams = this.extractParamForRegisterTeamsAppAndAad(
        ctx.answers
      );
  ...
  private prepareConfigForRegisterTeamsAppAndAad(
    ...
    config
      .get(GLOBAL_CONFIG)!
      .set(PERMISSION_REQUEST, JSON.stringify(DEFAULT_PERMISSION_REQUEST));
  ...
  private extractConfigForRegisterTeamsAppAndAad(
    ...
    { aadId: string; applicationIdUri: string; clientSecret: string },
    FxError
  > {
    const aadId = config
      .get(this.aadPlugin.name)
    ...
    if (
      applicationIdUri === undefined ||
      typeof applicationIdUri !== "string"
    ) {
      return err(
        returnSystemError(
  ...
  > {
    ...
    const domain = this.prepareConfigForRegisterTeamsAppAndAad(
      ctx.config,
      params
    );
    const aadPluginCtx = getPluginContext(
      ctx,
      this.aadPlugin.name,
      mockedManifest
    );
    ...
    const configResult = this.extractConfigForRegisterTeamsAppAndAad(
      ctx.config,
      isLocal
    );
    ...
    const appSettingsJSONTpl = (
      await fs.readFile(appSettingsJSONPath)
    ).toString();
    const maybeTenantId = this.parseTeamsAppTenantId(
      await ctx.appStudioToken?.getJsonObject()
    );
    ...
    if (isLocal) {
      ...
      const launchSettingsJSONTpl = (
        await fs.readFile(launchSettingsJSONPath)
      ).toString();
