...
import "mocha";
import { err, FxError, Result, ok, Inputs, Platform, Stage, SolutionContext, QTreeNode, Func, InputTextConfig, InputTextResult, SelectFolderConfig, SelectFolderResult, SingleSelectConfig, SingleSelectResult, OptionItem, traverse } from "@microsoft/teamsfx-api";
...
import { CoreQuestionNames, SampleSelect, ScratchOptionNoVSC, ScratchOptionYesVSC } from "../../src/core/question";
...
describe("Core basic APIs", () => {
    ...
    //setSubscriptionInfo
    {
      ...
      if(res2.isOk()){
        ...
        if(projectConfig !== undefined){
          ...
          assert.isTrue( sconfig !== undefined && sconfig.get("subscriptionId") === "000000-11111" && sconfig.get("tenantId") === "222222-33333");
        }
      } 
    }

    {
      ...
      if(res2.isOk()){
        ...
        if(projectConfig !== undefined){
          ...
          assert.isTrue(projectConfig.config !== undefined);
          const sconfig = projectConfig.config!.get("solution");
          assert.isTrue( sconfig !== undefined && sconfig.get("subscriptionId") === undefined && sconfig.get("tenantId") === undefined);
        }
      } 
    }
  });
  ...
  it("happy path: getQuestions, getQuestionsForUserTask for static question", async () => { 
    ...
    sandbox.stub<any, any>(mockSolution, "getQuestions").callsFake(async (task: Stage, ctx: SolutionContext) : Promise<Result<QTreeNode | undefined, FxError>> => {
      return ok(new QTreeNode({
        type: "text",
        name: "mock-question",
        title: "mock-question"
      }));
    });
    sandbox.stub<any, any>(mockSolution, "getQuestionsForUserTask").callsFake(async (func: Func, ctx: SolutionContext) : Promise<Result<QTreeNode | undefined, FxError>> => {
      return ok(new QTreeNode({
        type: "text",
