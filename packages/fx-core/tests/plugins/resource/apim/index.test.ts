...
describe("ApimPlugin", () => {
    let services: { aadService: AadService, aadHelper: AadHelper, apimService: ApimService, resourceGroupHelper: ResourceGroupHelper };
    let aadClientId = "";
    let aadObjectId = "";

    before_if(EnvConfig.enableTest, async () => {
        services = await buildService();
        await services.resourceGroupHelper.createResourceGroup(UT_RESOURCE_GROUP, EnvConfig.defaultLocation);
...
async function buildContext(resourceName: string, resourceNameSuffix: string, aadObjectId: string, aadClientId: string): Promise<PluginContext> {
...
async function buildService(): Promise<{ aadService: AadService, aadHelper: AadHelper, apimService: ApimService, resourceGroupHelper: ResourceGroupHelper }> {
    ...
    return { aadService: aadService, aadHelper: aadHelper, apimService: apimService, resourceGroupHelper: resourceGroupHelper };
}
