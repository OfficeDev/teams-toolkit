...
import { AzureSolutionSettings, ConfigMap, EnvConfig, FxError, Inputs, Json, ok, Platform, Plugin, PluginContext, ProjectSettings, Result, TokenProvider, Void } from "@microsoft/teamsfx-api";
import { FrontendPlugin, FrontendPluginV2, newEnvInfo } from "../../../../src";
import { provisionResourceAdapter, setConfigs, setLocalSettingsV1, setLocalSettingsV2, setProvisionOutputs } from "../../../../src/plugins/resource/utils4v2";
import { v1 } from "uuid";
import { MockAppStudioTokenProvider, MockAzureAccountProvider, MockGraphTokenProvider, MockTools, randomAppName } from "../../../core/utils";
import Container from "typedi";
import { ResourcePlugins, ResourcePluginsV2 } from "../../../../src/plugins/solution/fx-solution/ResourcePluginContainer";
...
describe("API V2 adapter", () => { 
  ...
  it("setProvisionOutputs", async () => {
    ...
    pluginContext.envInfo.profile.set("plugin1", new ConfigMap([["k1", "v1"],["k2", "v2"]]));
    pluginContext.envInfo.profile.set("plugin2", new ConfigMap([["k3", "v3"],["k4", "v4"]]));
  ...
  it("setLocalSettings", async () => {
    const pluginContext: PluginContext = {
      ...
      config: new ConfigMap(),
      envInfo: newEnvInfo(),
      localSettings: {teamsApp:new ConfigMap([["k1", "v1"]]), auth: new ConfigMap([["k2", "v2"]])},
    };
  ...
  it("provisionResourceAdapter", async () => {
    ...
    const res = await provisionResourceAdapter(context, inputs, provisionInputConfig, tokenProvider, plugin);
