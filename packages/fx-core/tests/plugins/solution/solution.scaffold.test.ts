...
describe("Solution scaffold() reading valid manifest file", () => {
  ...
  it("should work and generate arm template when project requires Azure services", async () => {
    ...
    expect(
      fileContent.has(path.join(testFolder, "./templates/azure/provision", "frontendHosting.bicep"))
    ).to.be.true;
    expect(
      fileContent.has(path.join(testFolder, "./templates/azure/provision", "simpleAuth.bicep"))
    ).to.be.true;
