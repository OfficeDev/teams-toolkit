...
2. Open appsettings.json located at `./src/TeamsFxSimpleAuth/appsettings.json` and update following configurations
    ...
    | AAD_METADATA_ADDRESS | Yes | https://login.microsoftonline.com/common/v2.0/.well-known/openid-configuration | Metadata document used during token validation. if AAD App is single tenant, replace common with {aad-tenant-id} |
