  ...
  it(
    ...
    async function () {
      ...
      const listCollaboratorResult = await execAsync(
        `teamsfx permission status --list-all-collaborators --env dev --teams-manifest-file ${projectPath}/appPackage/manifest.json --aad-manifest-file ${projectPath}/aad.manifest.json --interactive false`,
        {
          cwd: projectPath,
          env: process.env,
          timeout: 0,
