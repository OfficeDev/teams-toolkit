...
export async function addJsonFileAndParamtersFile(projectPath: string): Promise<void> {
  const srcFolder = path.join(__dirname, "../../asset/armDriver");
  const destFolder = path.join(projectPath, "infra");
  await fs.copyFile(path.join(srcFolder, jsonFileName), path.join(destFolder, jsonFileName));
  await fs.copyFile(
