export class TreatmentVariables {
  public static readonly VSCodeConfig = "vscode";
  public static readonly EmbeddedSurvey = "embeddedsurvey";
  public static readonly CustomizeTreeview = "customizetreeview";
  public static readonly QuickStartInSidebar = "quickstartinsidebar";
  public static readonly RefineScaffoldTooltip = "refinescaffoldhint";
  public static readonly RefineScaffoldOrder = "refinescaffoldorder";
  public static readonly ShowLocalDebug = "showlocaldebug";
}

export class TreatmentVariableValue {
  public static isEmbeddedSurvey: boolean | undefined = undefined;
  public static readonly TopSidebar: string = "topsidebar";
  public static readonly BottomSidebar: string = "bottomsidebar";
  public static readonly OriginalTreeView: string = "originaltreeview";
}
