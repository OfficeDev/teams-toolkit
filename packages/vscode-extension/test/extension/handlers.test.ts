...
import { vscodeLogger } from "../../src/debug/depsChecker/vscodeLogger";
...
describe("handlers", () => {
  ...
  describe("command handlers", function () {
    ...
    it("createNewProjectHandler - invoke Copilot", async () => {
      ...
      chai.assert.equal(executeCommandStub.args[0][1], "workbench.action.chat.open");
      sinon.restore();
    });
...
describe("invokeTeamsAgent", async () => {
  ...
  it("Install github copilot extension error", async () => {
    ...
    chai.assert.equal(loggerError.callCount, 1);
  });
