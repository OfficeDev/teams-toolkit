...
import { QuestionModelEngine, loadOptions } from "../src/visitor";
import {
  EmptyOptionsError,
...
describe("Question Model - Visitor Test", () => {
  ...
  describe("traverse()", () => {
    const engine = new QuestionModelEngine();
    ...
    it("success: node condition", async () => {
      ...
      const res = await engine.traverse(node1, inputs, mockUI);
      assert.isTrue(res.isOk());
    ...
    it("single selection", async () => {
      ...
      const inputs = createInputs();
      const res = await engine.traverse({ data: question }, inputs, mockUI);
      assert.isTrue(res.isOk());
    ...
    it("single selection empty options", async () => {
      ...
      if (res.isErr()) {
        assert.isTrue(res.error instanceof EmptyOptionsError);
      }
    });
  ...
  describe("defaultVisotor", () => {
    ...
    it("should return empty option error for non-interactive mode", async () => {
      ...
      const res = await engine.defaultVisotor(question, mockUI, inputs);
      assert.isTrue(res.isErr() && res.error instanceof EmptyOptionsError);
    });
    ...
    it("selectFiles", async () => {
      sandbox.stub(mockUI, "selectFiles").resolves(ok({ type: "success", result: ["a"] }));
    ...
    it("selectFileOrInput", async () => {
      sandbox.stub(mockUI, "selectFileOrInput").resolves(ok({ type: "success", result: "a" }));
      const question: SingleFileOrInputQuestion = {
        type: "singleFileOrText",
        name: "test",
        title: "test",
    ...
    it("confirm", async () => {
      sandbox.stub(mockUI, "confirm").resolves(ok({ type: "success", result: true }));
