...
export class TeamsFxMiddleware implements Middleware {
  ...
  constructor(options: TeamsFxMiddlewareOptions) {
    ...
    if (options.welcomeMessage) {
      ...
      this.welcomeMessage.trigger =
        options.welcomeMessage?.trigger ?? WelcomeMessageTrigger.BotInstall;
    }
  ...
  public async onTurn(context: TurnContext, next: () => Promise<void>): Promise<void> {
    ...
    switch (type) {
      ...
      case ActivityType.SettingCommandReceived:
        const card = await this.settingsProvider.sendSettingsCard(
          new TeamsFxBotContext(context, this.settingsStore)
        );
  ...
  private isSettingsCardSubmitted(activity: Activity): boolean {
    ...
    return (
      activity.value[this.settingsProvider.submitActionKey] ===
      this.settingsProvider.submitActionValue
    );
  }
}
