...
function sayCommand(feedbackLoopEnabled = false) {
    return async (context, _state, data) => {
        ...
        const contentText = citations.length < 1? content : Utilities.Utilities.formatCitationsResponse(content);
        // If there are citations, filter out the citations unused in content.
        const referencedCitations = citations.length > 0 ? Utilities.Utilities.getUsedCitations(contentText, citations) : undefined;
